...
public:
	...
	void addTupleToRelation( vector<Entry> tuple, Relation relationA ) {
		return addTupleToRelation( tuple, &relationA );
	}

	void addTupleToRelation( vector<Entry> tuple, string relationName ) {
		return addTupleToRelation( tuple, findRelation( relationName ) );
	}


	Relation* insertIntoFromRelation( Relation* relationA, Relation* relationB );
	Relation* insertIntoFromRelation( Relation* relationA, Relation relationB ) {
		return insertIntoFromRelation( relationA, &relationB );
	}

	Relation* insertIntoFromRelation( string relationA, Relation* relationB ) {
		return insertIntoFromRelation( findRelation( relationA ), relationB );
	}
	...
	Relation crossProduct( Relation* targetRelationA, string relationB ) {
		return crossProduct( targetRelationA, findRelation( relationB ) );
	}

	Relation crossProduct( string relationA, Relation* targetRelationB ) {
		return crossProduct( findRelation( relationA ), targetRelationB );
	}

	Relation crossProduct( string relationA, string relationB ) {
		return crossProduct( findRelation( relationA ), findRelation( relationB ) );
	}

	Relation crossProduct( Relation targetRelationA, Relation targetRelationB ) {
		return crossProduct( &targetRelationA, &targetRelationB );
	}
	...
	Relation deleteFromRelation( string relationA, vector<Condition> conditions ) {
		return deleteFromRelation( findRelation( relationA ), conditions );
	}

	Relation differenceTwoRelation( Relation* targetRelationA, Relation* targetRelationB );
	Relation differenceTwoRelation( Relation targetRelationA, Relation targetRelationB ) {
		return differenceTwoRelation( &targetRelationA, &targetRelationB );
	}

	Relation differenceTwoRelation( Relation* targetRelationA, string relationB ) {
		return differenceTwoRelation( targetRelationA, findRelation( relationB ) );
	}

	Relation differenceTwoRelation( string relationA, Relation* targetRelationB ) {
		return differenceTwoRelation( findRelation( relationA ), targetRelationB );
	}

	Relation differenceTwoRelation( string relationA, string relationB ) {
		return
			differenceTwoRelation( findRelation( relationA ), findRelation( relationB ) );
	}
	...
	Relation naturalJoin( Relation* targetRelationA, string relationB ) {
		return naturalJoin( targetRelationA, findRelation( relationB ) );
	}

	Relation naturalJoin( string relationA, Relation* targetRelationB ) {
		return naturalJoin( findRelation( relationA ), targetRelationB );
	}

	Relation naturalJoin( string relationA, string relationB ) {
		return naturalJoin( findRelation( relationA ), findRelation( relationB ) );
	}


	Relation projection( vector<string> attributeNames, Relation* targetRelation );
	Relation projection( vector<string> attributeNames, Relation targetRelation ) {
		return projection( attributeNames, &targetRelation );
	}

	Relation projection( vector<string> attributeNames, string relationName ) {
		return projection( attributeNames, findRelation( relationName ) );
	}
	...
	Relation renameAttributes( vector<string> newNames, Relation targetRelation ) {
		return renameAttributes( newNames, &targetRelation );
	}

	Relation renameAttributes( vector<string> newNames, string relationName ) {
		return renameAttributes( newNames, findRelation( relationName ) );
	}
	...
	Relation selection( vector<Condition> conditions, Relation targetRelation ) {
		return selection( conditions, &targetRelation );
	}

	Relation selection( vector<Condition> conditions, string targetRelationName ) {
		return selection( conditions, findRelation( targetRelationName ) );
	}

	Relation unionTwoRelations( Relation* targetRelationA, Relation* targetRelationB );
	Relation unionTwoRelations( Relation targetRelationA, Relation targetRelationB ) {
		return unionTwoRelations( &targetRelationA, &targetRelationB );
	}
	...
	Relation unionTwoRelations( string relationA, string relationB ) {
